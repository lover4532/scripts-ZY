...
# filename = os.path.join(dirname, 'mistep')
# mistepfile = Path(filename)
...
# 主函数
def main(_user, _passwd, _maxstep=16000):
    result = f"[{now}]\n{_user[:3]}****{_user[-4:]}:\n"
    print(result)
    ...
    if _maxstep <= preStep :
        ...
        f.close()
        return f"{result}今天目标步数：{_maxstep},已经达到！"
    
    f.seek(0)
    f.truncate()

    addStep = random.randint(1, 1800)
    _step=preStep+addStep
    
    if localtime.tm_hour>=22:
        _step = _maxstep
    elif _step>_maxstep:
        _step = _maxstep
        # list1 = [str(random.randint(1, time.localtime().tm_hour)),str("{:0>2d}".format(random.randint(1, time.localtime().tm_min))),str(random.randint(1,9))]
        # _step = "".join(list1)

        # print("已设置为随机步数（18000-25000）")
        # _step = str(random.randint(18000, 25000))
    
    # print(f"当前步数为：{_step};目标步数为：{_maxstep}")
    f.write(f"{date},{_maxstep},{_step}")
    f.close()

    login_token, userid = login(_user, password)
    if login_token == 0:
        print("登陆失败！")
        return f"{result}login fail!"
    ...
    data_json = re.sub(find_step.findall(data_json)[0], str(_step), str(data_json))
...
if __name__ ==  "__main__":
    ...
    if len(user_list) == len(passwd_list):
        ...
        for line in range(0,len(user_list)):
            if len(step_array) == 2:
                step = random.randint(int(step_array[0]),int(step_array[1]))
