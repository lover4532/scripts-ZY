...
const $ = new Env("中青看点极速版阅读文章");
const notify = $.isNode() ? require('./sendNotify') : '';
message = ""


let spwzbody= $.isNode() ? (process.env.spwzbody ? process.env.spwzbody : "") : ($.getdata('spwzbody') ? $.getdata('spwzbody') : "")
let spwzbodyArr = []
let spwzbodys = ""
...
if (spwzbody) {
    if (spwzbody.indexOf("&") == -1) {
        spwzbodyArr.push(spwzbody)
    } else if (spwzbody.indexOf("&") > -1) {
        spwzbodys = spwzbody.split("&")
    } else if (process.env.spwzbody && process.env.spwzbody.indexOf('&') > -1) {
        spwzbodyArr = process.env.spwzbody.split('&');
        console.log(`您选择的是用"&"隔开\n`)
    }
} else if($.isNode()) {
    var fs = require("fs");
    spwzbody = fs.readFileSync("spwzbody.txt", "utf8");
    if (spwzbody !== `undefined`) {
        spwzbodys = spwzbody.split("\n");
    } else {
...
Object.keys(spwzbodys).forEach((item) => {
    if (spwzbodys[item] && !spwzbodys[item].startsWith("#")) {
        spwzbodyArr.push(spwzbodys[item])
    }
})
...
!(async () => {
    if (typeof $request !== "undefined") {
     ...
     $.done()
        ...
        console.log(`共${spwzbodyArr.length}个阅读body`)
        for (let k = 0; k < spwzbodyArr.length; k++) {
            // $.message = ""
            spwzbody1 = spwzbodyArr[k];
            // console.log(`${spwzbody1}`)
...
function wzjl(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            url : 'https://user.youth.cn/FastApi/article/complete.json?'+spwzbody1+'&app_version=2.4.5',
            headers : wzheader,
            }//xsgbody,}
