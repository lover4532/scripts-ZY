...
if (zqspeedckArr.length <= 0) {
    ...
    !(async () => {
        console.log(`共${zqspeedckArr.length}个账号`)
        for (let k = 0; k < zqspeedckArr.length; k++) {
            $.message = ""
            var articleId = await getArticle(zqspeedckArr[k])
            await shareArticleCallback(zqspeedckArr[k])
            await $.wait(2000);
            await shareEnd(zqspeedckArr[k],articleId)
            console.log("\n转发成功，可以去提现啦~\n")
        }
        ...
        //     if (message.length != 0) {
        //         await notify.sendNotify("中青看点极速版转发提现", `${message}\n\n shaolin-kongfu`);
        //     }
        // } else {
        //     $.msg($.name, "", message)
        // }
...
function shareArticleCallback(ck, timeout = 0) {
    return new Promise((resolve,reject) => {
        let url = {
            url: `https://user.youth.cn/FastApi/ArticleTop/shareArticleCallback.json?${ck}&from=1`,
            headers: qdheader,
        }
        $.get(url, async (err, resp, data) => {
            try {
...
function shareEnd(ck,articleId, timeout = 0) {
    return new Promise((resolve,reject) => {
        let url = {
            url: `https://user.youth.cn/FastApi/article/shareEnd.json?${ck}&from=1&device_platform=android&article_id=${articleId}&stype=WEIXIN_CIRCLE`,
            headers: qdheader,
        }
        $.get(url, async (err, resp, data) => {
            try {
