...
*/
const $ = new Env("中青看点极速版-CK");
const apiUrl = 'https://user.youth.cn/v1/user/userinfo.json' //重写目标 zq_speed_ck.js
...
if (typeof $request !== "undefined") {
    ...
    if (zqspeedckArr.length <= 0) {
        $.msg($.name, '【提示】请点击身份以获取body，明天再跑一次脚本测试', '不知道说啥好', {
            "open-url": "给您劈个叉吧"
        });
        $.done()
        ...
        !(async () => {
            console.log(`共${zqspeedckArr.length}个身份`)
            for (let k = 0; k < zqspeedckArr.length; k++) {
                $.message = ""
                // console.log(`${zqspeedck1}`)
                console.log(`--------第 ${k + 1} 个身份任务执行中--------\n`)
                await dozqspeedck(zqspeedckArr[k])
                await $.wait(1000);
                console.log("\n\n")
            }
            ...
            if ($.isNode() && date.getHours() == 11 && date.getMinutes() < 10) {
                if (message.length != 0) {
                    await notify.sendNotify("中青看点极速版身份", `${message}\n\n shaolin-kongfu`);
                }
            } else {
                $.msg($.name, "", message)
            }
...
//获取身份body
function getzqspeedck() {
    if ($request.url.indexOf(apiUrl) != -1) {
        var bodyVal = getQueryPara($request.url)
        bodyVal = bodyVal.replace(/is_add_desktop=[\w]\&/g,'')
        var bodyObj = parseQueryParas(bodyVal)
        $.log(`${$.name}获取身份: 成功, zqspeedcks: ${bodyVal}`);
        // if (zqspeedck) {
        var findIndex = isExists(bodyObj.uid)
        if (findIndex != -1) {
            zqspeedckArr[findIndex] = bodyVal
            $.log("此身份请求已存在，本次将更新")
        } else
            zqspeedckArr.push(bodyVal)

        $.setdata(zqspeedckArr.join('\n'), 'zqspeedck');
        $.msg($.name, "获取第" + zqspeedckArr.length + "个身份请求: 成功🎉", ``)
    }
}

//身份
function dozqspeedck(ck, timeout = 0) {
            ...
            try {
                ...
                if (result.success == true) {
                    if (typeof result.items.score === 'undefined')
                        ...
                        console.log('\n开身份成功，获得：' + result.items.score + '金币')
                } else {
                    console.log('\n身份已经开启，等下再来吧^_^')
                }
            } catch (e) {
...
function isExists(uid) {
    var strUid = `uid=${uid}`
    for (let index = 0; index < zqspeedckArr.length; index++) {
        const element = zqspeedckArr[index];
